...
import (
	...
	"Broker_backend/services/integration/brokerservice/internal/infra/clock"
	postgres2 "Broker_backend/services/integration/brokerservice/internal/infra/repositories/postgres"
	"Broker_backend/services/integration/brokerservice/internal/infra/repositories/postgres/features/fixations"
	"Broker_backend/services/integration/brokerservice/internal/transport/brokerhandler"
...
func Run() error {
	...
	pool, err := postgres2.NewPool(pgCtx, cfg)
	if err != nil {
	...
	txManager := postgres2.NewTxManager(pool) // ← нужен конструктор, см. txmanager_patch.go
	...
	//  usecases.FixationRepository ниже — Go проверит методы сам.
	// ═══════════════════════════════════════════════════════════════════
	fixationRepo := fixations.NewRepository(txManager)
	...
	// ═══════════════════════════════════════════════════════════════════
	brokerService := usecases.NewService(
		cfg,
		logger,
		realClock,    // → интерфейс usecases.Clock
		fixationRepo, // → интерфейс usecases.FixationRepository
		txManager,    // → интерфейс usecases.TxManager
	)
	...
	brokerv1.RegisterBrokerServiceServer(
		grpcServer,
		brokerhandler.NewHandler(brokerService, logger),
	)
	...
	go func() {
		logger.Info("brokerservice grpc started", zap.String("addr", addr))
	...
	case <-rootCtx.Done():
		...
		select {
		case <-stopped:
			logger.Info("brokerservice stopped gracefully")
