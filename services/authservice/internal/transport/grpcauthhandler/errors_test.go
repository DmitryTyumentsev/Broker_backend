...
func TestMapErrorSanitizesWrappedErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			err:     fmt.Errorf("usecases.Register: %w", usecases.ErrEmailInvalid),
			code:    codes.InvalidArgument,
			message: usecases.ErrEmailInvalid.Error(),
		},
		{
			name:    "unique email error",
			err:     fmt.Errorf("usecases.Register: save user: postgres.users.Save: %w", domain.ErrNotUniqueEmail),
			code:    codes.AlreadyExists,
			message: "email already exists",
		},
		{
			name:    "auth error",
			err:     fmt.Errorf("usecases.Login: %w", domain.ErrUnauthenticated),
