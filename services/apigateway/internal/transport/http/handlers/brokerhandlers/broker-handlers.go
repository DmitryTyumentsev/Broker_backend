...
type BrokerHandler struct {
	logger     *zap.Logger
	grpcClient *brokerclient.GRPCClient
	httpClient *brokerclient.HTTPClient
	validator  *validate.Validate
}

func NewBrokerHandler(logger *zap.Logger, client *brokerclient.GRPCClient, validator *validate.Validate) *BrokerHandler {
...
func (h *BrokerHandler) NewFixationCustomer(c *fiber.Ctx) error { //Верно понял что согласно моему миддлвару аксесс лог, каждый(вообще каждый) вызов всех методов из цепочки очень подробно записывается и трейсится ещё плюсом?
	...
	protoResp, err := h.grpcClient.NewFixationCustomer(ctx, protoDTO)
	if err != nil {
		middleware.AuditLog(
			c,
			h.logger,
...
func (h *BrokerHandler) UpdateFixationCustomer(c *fiber.Ctx) error {
	...
	ctx := c.UserContext()
	dtoResp, err := h.httpClient.UpdateFixation(ctx, &dtoReq, principal.BrokerID, principal.UserID)
	if err != nil {
